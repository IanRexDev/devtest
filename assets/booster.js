...
class BstrCart{
    ...
    async clear(){
        let r = await fetch('/cart/clear.js')
        if(!r.ok) return
    ...
    async dispatchListeners({change = false} = {}){
        ...
        if(time > self.minicart.lastUpdate){
            let cartData = await (await fetch('/?section_id=api__minicart', {headers: { 'Pragma': 'no-cache', 'Cache-Control': 'no-cache, no-store, must-revalidate', 'Expires': 0 }})).text();
    ...
    async removeFromCart(target = false, id){
        ...
        let res = await (await fetch('/cart/change.js', {method: 'POST', headers: {'Content-Type': 'application/json' }, body: JSON.stringify({quantity: 0, id: id})})).json();
    ...
    async updateQuantity(event, variant, quantity, forced = false){
        ...
        let res = await (await fetch('/cart/change.js', {method: 'POST', headers: {'Content-Type': 'application/json' }, body: JSON.stringify({quantity: quantity, id: variant})})).json();
    ...
    async processQueue(data){
        ...
        let res = await (await fetch('/cart/add.js', fconfig)).json();
    ...
    success(data){
        ...
        if(config.action == 'cart') return window.location = '/cart';
        if(config.action == 'checkout') return window.location = '/checkout';
    ...
    async localize(params){
        ...
        if(mode != 'none' && currencies.includes(currency)){
            if(mode == 'convert' && bstore.get('currentCurrency') != currency){
                ...
                await fetch('/cart/update', {body: form, method: 'POST'});
                if(!redirect){
...
class BstrElements{
    ...
    checkPagination(){
        if(_settings.pagination == "infinite"){
            ...
            window.addEventListener('scroll', debounce( ()=>{
                if((window.innerHeight + window.scrollY) >= pList.offsetHeight){
    ...
    searchSuggest(event){
        ...
        if(query.length > 0){
            let q = `/search/suggest.json?q=${query}&resources[type]=product&resources[limit]=5&resources[options][unavailable_products]=last`;
            fetch(q)
            .then(r=>r.json())
    ...
    toggleTab(self, id){
      ...
      async renderChangePage(e = false, clickTarget, overwrite = false){
        ...
        let {html} = await this.basync.load({url, section: 'product-list', selector: '.product__list.row--product'})
        if(!html){
            container.style.opacity = '1'
      ...
      localizeDates(){
        ...
        for(let date of dates){
            ...
            date.innerText = cdate.toGMTString().split(cdate.getFullYear())[0].trim()
        }
    }
}
...
class ProductSlider extends BstrSlider {
    ...
    initThumbs(){
        ...
        for(let thumb of thumbs){
            ...
            thumb.addEventListener('click', ()=>this._moveTo({slider, index}))
            if(thumb.getAttribute('data-bstr-slider-thumb-hover') != null) thumb.addEventListener('mouseover', ()=>this._moveTo({slider, index}))
        }
    }
...
class BstrSectionSlider extends BstrSlider{
    ...
    _moveCallback(params){
        ...
        if(params.slider.dataset.sliderHolder == "adaptive"){
            params.slider.style.height = `${params.current[0].target.offsetHeight}px`;
        }
    }
}
